    ...
    <header className="w-full bg-white/80 dark:bg-gray-900/80 backdrop-blur-md shadow-sm border-b border-gray-100 dark:border-gray-700 sticky top-0 z-50">
      <nav className="max-w-7xl mx-auto px-6 md:px-8 py-3 flex items-center justify-between">
        ...
        >
          {/* Hamburger Icon */}
          <svg
            className={`w-6 h-6 transition-all duration-200 text-gray-7
            00 dark:text-gray-200 ${open ? "scale-0 opacity-0" : "scale-100 opacity-100"}`}
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
            ...
            <div className="relative">
              ...
              <div
                className={`ml-2 mt-1 space-y-1 overflow-hidden transition-all duration-300 ${
                  loginDropdownOpen ? "max-h-40 opacity-100" : "max-h-0 opacity-0"
                }`}
              >
                {loginItems.map(({ to, label }) => (
                  <a
